...
import { Award, GraduationCap, LayoutDashboard, LogOut, UserRound } from "lucide-react";
...
const navItems = [
  { href: "/student", key: "overview", icon: LayoutDashboard, exact: true },
  { href: "/student/certificates", key: "certificates", icon: Award, exact: false },
  { href: "/student/profile", key: "profile", icon: UserRound, exact: false },
] as const;
      ...
      <aside className="flex h-full w-[280px] shrink-0 flex-col border-r border-slate-200 bg-white/95 dark:border-slate-800 dark:bg-slate-900/95">
        <div className="border-b border-slate-200 bg-sky-50 p-6 dark:border-slate-800 dark:bg-sky-500/10">
          ...
          <h2 className="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">
            {t("studentPortal.shell.consoleTitle")}
          </h2>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
            {t("studentPortal.shell.consoleDescription")}
          </p>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1">{t(`studentPortal.shell.${item.key}`)}</span>
              </Link>
            );
          })}
        </nav>
        ...
        <div className="space-y-2 border-t border-slate-200 p-4 dark:border-slate-800">
          ...
          >
            ...
            <span>{t("studentPortal.shell.logout")}</span>
          </button>
        </div>
      </aside>
