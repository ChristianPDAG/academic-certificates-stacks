...
type ActionState = { id: string; chainId: number; action: "revoke" | "reactivate" } | null;
...
export function CertificatesTable({
  ...
  const [actionCert, setActionCert] = useState<ActionState>(null);
  const [processing, setProcessing] = useState(false);

  const allSelected = useMemo(() => {
    return (
      certificates.length > 0 &&
      certificates.every((c) => c.chain_cert_id && selectedIds.has(c.id_certificate))
    );
  }, [certificates, selectedIds]);

  const someSelected = useMemo(() => {
    const selectable = certificates.filter((c) => !!c.chain_cert_id);
    if (selectable.length === 0) return false;
    const count = selectable.filter((c) => selectedIds.has(c.id_certificate)).length;
  ...
  const SortIcon = ({ field }: { field: keyof CertificateRow }) => {
    if (sortField !== field) return <ChevronsUpDown className="h-4 w-4 text-neutral-400" />;
            ...
            <tr>
              <th className="px-4 py-3 text-left">
                <Checkbox
                  ...
                  // We render partial via opacity; if you prefer true indeterminate, set checked="indeterminate"
                  onCheckedChange={(val) => handleSelectAll(!!val)}
                  className={someSelected ? "opacity-50" : ""}
                  aria-label={t("academy.certificates.table.selectAll")}
                />
              </th>

              <SortableHeader field="chain_cert_id">
                {t("academy.certificates.table.id")}
              </SortableHeader>
              ...
              <SortableHeader field="created_at">
                {t("academy.certificates.table.date")}
              </SortableHeader>
          ...
          <tbody className="bg-white dark:bg-neutral-950 divide-y divide-neutral-200 dark:divide-neutral-800">
            {loading ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center">
                  <div className="flex items-center justify-center gap-2 text-neutral-600 dark:text-neutral-300">
                    ...
                    <span>{t("academy.certificates.table.loading")}</span>
                  </div>
                </td>
              </tr>
            ) : certificates.length === 0 ? (
              <tr>
                ...
                >
                  {t("academy.certificates.table.noCertificates")}
                </td>
              </tr>
            ) : (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm font-mono text-neutral-900 dark:text-neutral-100">
                    {cert.chain_cert_id ? (
                      <div className="flex items-center gap-2">
                        ...
                        {cert.tx_id && (
                          <a
                            ...
                            aria-label={t("academy.certificates.table.openTx")}
                            title={t("academy.certificates.table.openTx")}
                          >
                      ...
                      <span className="text-neutral-400">
                        {t("academy.certificates.table.notAvailable")}
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs font-mono text-neutral-400 dark:text-neutral-500 truncate max-w-[220px]">
                        {cert.student_wallet}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col">
                      <span className="text-sm text-neutral-900 dark:text-neutral-100">
                        {cert.id_course?.title ?? t("academy.certificates.table.noTitle")}
                      </span>
                      {cert.grade && (
                        <span className="text-xs text-neutral-500 dark:text-neutral-400">
                          {t("academy.certificates.table.grade", { grade: cert.grade })}
                        </span>
                      )}
                    </div>
                  </td>
                      ...
                      {cert.chain_cert_id ? (
                        <>
                          <Button
                            ...
                            title={t("academy.certificates.table.sync")}
                            aria-label={t("academy.certificates.table.sync")}
                          >
                          ...
                          {cert.status === "issued" ? (
                            <Button
                              ...
                              title={t("academy.certificates.table.revoke")}
                              aria-label={t("academy.certificates.table.revoke")}
                            >
                            ...
                            <Button
                              ...
                              title={t("academy.certificates.table.reactivate")}
                              aria-label={t("academy.certificates.table.reactivate")}
                            >
                        ...
                        <span className="text-xs text-neutral-400 dark:text-neutral-500 flex items-center gap-1">
                          ...
                          {t("academy.certificates.table.noBlockchain")}
                        </span>
                      )}
                    </div>
                  </td>
          ...
          <AlertDialogHeader>
            <AlertDialogTitle>
              {actionCert?.action === "revoke"
                ? t("academy.certificates.dialogs.revokeTitle")
                : t("academy.certificates.dialogs.reactivateTitle")}
            </AlertDialogTitle>
            ...
            >
              {processing ? (
                <>
                  ...
                  {actionCert?.action === "revoke"
                    ? t("academy.certificates.dialogs.revoking")
                    : t("academy.certificates.dialogs.reactivating")}
                </>
              ) : actionCert?.action === "revoke" ? (
