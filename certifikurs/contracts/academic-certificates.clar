...
(define-public (add-school
    ...
    (begin
        ...
        ;; Verificar que la escuela no sea el super-admin
        (asserts! (not (is-eq school-principal (var-get super-admin))) ERR_NOT_AUTHORIZED)
        ...
        ;; Verificar que el nombre de la escuela no este vacio
        (asserts! (> (len school-name) u0) ERR_NOT_AUTHORIZED)

        ;; Agregar la escuela
        (ok (map-set authorized-schools { school-principal: school-principal } {
...
(define-public (issue-certificate
    ...
    (let ((new-cert-id (get-next-certificate-id)))
        ...
        (asserts! (> (len student-id) u0) ERR_NOT_AUTHORIZED)
        (asserts! (> (len course) u0) ERR_NOT_AUTHORIZED)
        (asserts! (> (len grade) u0) ERR_NOT_AUTHORIZED)
        
        ;; Validar que student-wallet no sea el super-admin ni la escuela emisora
        (asserts! (not (is-eq student-wallet (var-get super-admin))) ERR_NOT_AUTHORIZED)
...
(define-public (change-super-admin (new-admin principal))
    (begin
        (asserts! (is-super-admin) ERR_NOT_AUTHORIZED)
        (asserts! (not (is-eq new-admin (var-get super-admin))) ERR_NOT_AUTHORIZED)
